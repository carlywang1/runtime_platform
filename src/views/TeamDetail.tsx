...
const initialEnvConfigs: Record<string, EnvConfig[]> = {
  'team-5': [
    {
      id: 'env-prod',
      ...
      isActive: true,
      fields: [
        { label: 'Outlook Tenant ID', key: 'tenant_id', value: 'prod-a1b2c3d4-e5f6-7890-abcd-ef1234567890', required: true },
        { label: 'Client ID', key: 'client_id', value: 'prod-x9y8z7w6-v5u4-t3s2-r1q0-p9o8n7m6l5k4', required: true },
        { label: 'Mailbox', key: 'mailbox', value: 'cs@acme-prod.com', required: true },
        { label: 'Manager Email', key: 'manager_email', value: 'manager@acme-prod.com', required: true },
      ],
    },
  ],
};
...
const mockTeamsData: Record<string, {
  ...
  'team-5': {
    ...
    pendingApprovals: 0,
    agents: [
      { id: 'e1', name: 'Customer Service Email Assistant', emoji: '📧', role: 'Agent', status: 'idle', currentTask: null },
    ],
  },
};
...
export default function TeamDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [envConfigs, setEnvConfigs] = useState<EnvConfig[]>(initialEnvConfigs[id] || []);
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <Link href="/teams" className="text-indigo-400 hover:text-indigo-300">Return to Teams</Link>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen">
      ...
      <div className="p-6 space-y-6">
        ...
        <div className="flex items-center gap-4">
          <Link href="/teams" className="p-2 rounded-lg hover:bg-white/5 transition-colors">
              ...
              {team.agents.map((agent) => (
                <div key={agent.id} className="p-4 rounded-xl bg-black/60 border border-white/10">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <Link href={`/agents/${agent.id}`} className="text-xs text-indigo-400 hover:text-indigo-300">View Details →</Link>
                  </div>
                </div>
              ))}
            </div>
        ...
        {activeTab === 'config' && (
          <div className="space-y-4">
            {envConfigs.length === 0 ? (
              <div className="bg-dark-50 rounded-2xl border border-white/5 p-6">
                <h3 className="text-lg font-semibold text-white mb-4">Config</h3>
                <p className="text-gray-400">No environment configurations yet.</p>
              </div>
            ) : (
              envConfigs.map((cfg) => {
                ...
                return (
                  <div
                    key={cfg.id}
                    className="bg-dark-50 rounded-2xl border border-emerald-500/30 border-l-[3px] border-l-emerald-500 p-6"
                  >
                    {/* Header */}
                    <div className="flex items-center justify-between mb-4">
                      <div className="flex items-center gap-3">
                        <h3 className="text-base font-semibold text-white">{cfg.name}</h3>
                        <span className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-[11px] font-medium bg-emerald-500/20 text-emerald-400">
                      ...
                      {!isEditing && (
                        <button
                          onClick={() => {
                            ...
                            cfg.fields.forEach((f) => { draft[f.key] = f.value; });
                            setEditDraft(draft);
                          }}
                          className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium text-gray-400 hover:text-indigo-300 hover:bg-indigo/10 transition-colors"
                        >
                    ...
                    <div className="space-y-2.5">
                      {cfg.fields.map((field) => (
                        <div key={field.key} className="py-2.5 px-3.5 rounded-lg bg-black/40">
                          <div className="flex items-center gap-2 mb-1">
                            <span className="text-xs font-medium text-gray-300">{field.label}</span>
                            {field.required && (
                              <span className="px-1.5 py-0.5 rounded text-[9px] font-semibold bg-emerald-500/15 text-emerald-400">REQUIRED</span>
                            )}
                          </div>
                          {isEditing ? (
                            <input
                              ...
                              className="w-full h-8 px-2.5 rounded-lg bg-white/[0.03] border border-white/[0.08] text-xs text-white placeholder-gray-600 focus:outline-none focus:border-indigo/40 transition-colors"
                            />
                          ) : (
                        ...
                        <button
                          onClick={() => {
                            setEnvConfigs((prev) =>
                              prev.map((c) =>
                                c.id === cfg.id
                                  ? { ...c, fields: c.fields.map((f) => ({ ...f, value: editDraft[f.key] ?? f.value })) }
                                  : c
                              )
                            );
                          ...
                          className="px-4 py-2 rounded-lg text-xs font-semibold text-white bg-gradient-to-r from-indigo to-[#7F43AD] hover:opacity-90 transition-opacity"
                        >
                          Save
                        </button>
                        <button
        ...
        {(activeTab === 'tasks' || activeTab === 'approval_policy' || activeTab === 'skills' || activeTab === 'metrics') && (
          <div className="bg-dark-50 rounded-2xl border border-white/5 p-6">
